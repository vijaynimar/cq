...
// const senderEmail = process.env.BREVO_SENDER_EMAIL || "fitnessbuddy08@gmail.com";
// const brevoApiKey = process.env.BREVO_API_KEY;

// if (!brevoApiKey) {
//   console.warn("BREVO_API_KEY is not set. OTP emails will fail until it is configured.");
// }
...
if (brevoApiKey.startsWith("xsmtpsib-")) {
  console.warn("BREVO_API_KEY appears to be an SMTP key (xsmtpsib-...). For Brevo SDK/API calls, use an API key starting with xkeysib-.");
}
...
// ✅ UPDATED: Registration OTP Email (Brevo API)
const sendRegistrationOtpEmail = async ({ email, firstName, otp }) => {
  try {
    ...
    console.error("Error sending registration OTP email:", err?.response?.body || err);
    throw new Error("Failed to send registration OTP email");
  }
};

// ✅ UPDATED: Reset Password Email (Brevo API)
const sendResetPasswordEmail = async ({ email, firstName, otp }) => {
  try {
    console.log(`Sending password reset email to ${email} with OTP ${otp}`);

    if (brevoApiKey.startsWith("xsmtpsib-")) {
      throw new Error("Invalid Brevo key type in BREVO_API_KEY. Use API key (xkeysib-...), not SMTP key (xsmtpsib-...).");
    }
